...
public class EventPersistorMetrics<V> {
    private static final Gauge GAUGE_QUEUE_TASK_CAPACITY = Gauge
            .build("th2_estore_persistor_queue_task_capacity", "Events queue size").register();

    private static final Gauge GAUGE_QUEUE_USED_TASK_COUNT = Gauge
            .build("th2_estore_persistor_queue_task_count", "Number of events queued for persistence").register();

    private static final Gauge GAUGE_QUEUE_FREE_TASK_COUNT = Gauge
            .build("th2_estore_persistor_queue_free_tasks", "Number of events that can be queued").register();

    private static final Gauge GAUGE_QUEUE_MAX_DATA_SIZE = Gauge
            .build("th2_estore_persistor_queue_max_data_size", "Max data size that can be queued").register();

    private static final Gauge GAUGE_QUEUE_USED_DATA_SIZE = Gauge
            .build("th2_estore_persistor_queue_used_data_size", "Data size of queued events").register();

    private static final Gauge GAUGE_QUEUE_FREE_DATA_SIZE = Gauge
            .build("th2_estore_persistor_queue_free_data_size", "Available data size").register();

    private static final Counter COUNTER_EVENTS_PERSISTED = Counter
            .build("th2_estore_persistor_events_persisted", "Number of events persisted").register();

    private static final Counter COUNTER_EVENTS_SIZE_PERSISTED = Counter
            .build("th2_estore_persistor_events_sizes_persisted", "Content size of events that persisted").register();

    private static final Counter COUNTER_PERSISTENCE_FAILURES = Counter
            .build("th2_estore_persistor_persistence_failures", "Number of event persistence failures").register();

    private static final Counter COUNTER_ABORTED_PERSISTENCES = Counter
            .build("th2_estore_persistor_aborted_persistences", "Number of aborted event persistences").register();

    private static final Counter COUNTER_1ST_RETRIES = Counter
            .build("th2_estore_persistor_1st_retries", "Number of primary persistence retries").register();

    private static final Counter COUNTER_2ND_RETRIES = Counter
            .build("th2_estore_persistor_2nd_retries", "Number of secondary persistence retries").register();

    private static final Counter COUNTER_FURTHER_RETRIES = Counter
            .build("th2_estore_persistor_further_retries", "Number of further persistence retries").register();

    private static final Histogram HISTOGRAM_PERSISTENCE_LATENCY = Histogram
            .build("th2_estore_persistor_persistence_latency", "Event persistence latency")
    ...
    public EventPersistorMetrics(BlockingScheduledRetryableTaskQueue<V> taskQueue) {
